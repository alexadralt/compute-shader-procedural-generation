...
float noise(float2 coords)
{
    ...
    coords /= (terrain_generation_settings.frequency * terrain_generation_settings.terrain_size);
    coords -= 0.5;
    
    float value = smax(abs(coords.x - coords.y), abs(coords.x + coords.y), 0.05);
    return clamp(value, 0, 1) * 2.0 - 1.0;
}
